...
int main(){
    ...
    //Εμφάνισε τα στοιχεία του δέντρου σε αύξουσα διάταξη (ενδοδιατεταγμένη διάσχιση)
    printf("Elements of BST in increasing order\n");
    RecBSTInorder(ARoot);
    //Εμφάνισε το αποτέλεσμα που επιστρέφει η RightNodeCount(…)
    printf("\nRightNodeCount = %d\n",RightNodeCount(ARoot));

return 0;}

int RightNodeCount(BinTreePointer Root) //BinTreePointer *
{
    int lcount, rcount;
    //Αρχικοποίησε τα lcount και rcount
    lcount=0;
    rcount=0;

    //Αν το ΔΔΑ είναι κενό (BSTEmpty(…))
    if(BSTEmpty(Root))
        return 0;
    //Αν το αριστερό υποδέντρο της ρίζας δεν είναι κενό
    if(!BSTEmpty(Root->LChild))
        //ανάθεσε στην lcount το αποτέλεσμα της κλήσης της RightNodeCount για το αριστερό υποδέντρο
        lcount=RightNodeCount(Root->LChild);

    //Αν το δεξί υποδέντρο της ρίζας δεν είναι κενό
    if(!BSTEmpty(Root->RChild))
        //ανάθεσε στην rcount το αποτέλεσμα της κλήσης της RightNodeCount για το αριστερό δεξί
        rcount=1+RightNodeCount(Root->RChild);
        //υποδέντρο+1

    //Επέστρεψε το άθροισμα των lcount και rcount
