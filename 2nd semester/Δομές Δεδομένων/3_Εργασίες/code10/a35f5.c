...
int main(){
    BinTreePointer ARoot, node1, node2,loc;
    BinTreeElementType n,num1, num2, AnItem;
    //Δήλωσε & Αρχικοποίησε found;
    boolean found;
    found=FALSE;
    //Δήλωσε άλλες μεταβλητές
    int num,i;
    //Δημιουργία κενού ΔΔΑ (CreateBST(…))
    CreateBST(&ARoot);

    //Διάβασε το πλήθος των στοιχείων
    printf("Enter number of elements for the tree:");
    scanf("%d",&num);

    //Επαναληπτικά για το πλήθος των στοιχείων
    for(i=0;i<num;i++){
        //Διάβασε τον αριθμό για εισαγωγή στο ΔΔΑ
        printf("Enter number:");
        scanf("%d",&n);
        //Εισήγαγε τον αριθμό στο ΔΔΑ (BSTInsert(…))
        RecBSTInsert(&ARoot,n);
    }

    //Εμφάνισε τα στοιχεία του ΔΔΑ σε αύξουσα διάταξη (ενδοδιατεταγμένη διάσχιση)
    ...
    //Επαναληπτικά
    do{
         //Διάβασε τον αριθμό num1;
         printf("Give first element of BST:");
         scanf("%d",&num1);
         //Διάβασε τον αριθμό num2;
         printf("Give second element of BST:");
         scanf("%d",&num2);

         //Ελεγχος αν το num1 είναι στοιχείου ΔΔΑ (RecBSTSearch(…));
         RecBSTSearch(ARoot,num1, &found, &node1);

         //Αν βρέθηκε το num1
         if(num1==found)
            //Ελεγχος αν το num2 είναι στοιχείου ΔΔΑ (RecBSTSearch(…));
            RecBSTSearch(ARoot,num2, &found, &node2);
    } //όσο δε βρέθηκε;
    while(!found);

    //Εμφάνισε την τιμή του κόμβου που επιστρέφει η κλήση της LCANode(…)
    printf("LCA node of %d and %d is node with element %d\n",num1,num2,LCANode(ARoot,node1,node2));
return 0;
}

BinTreePointer LCANode(BinTreePointer Root, BinTreePointer node1, BinTreePointer node2) {
    //Αν η τιμή της ρίζας του ΔΔΑ είναι μεγαλύτερη από τους δύο κόμβους
    if(Root > node1 && Root > node2)
        //Επέστρεψε την τιμή της συνάρτησης LCA για το αριστερό υποδέντρο της ρίζας.
        return LCANode(Root->LChild, node1, node2);
    //Αν η τιμή της ρίζας του ΔΔΑ είναι μικρότερη από τους δύο κόμβους
    if(Root < node1 && Root < node2)
        //Επέστρεψε την τιμή της συνάρτησης LCA για το δεξί υποδέντρο της ρίζας.
